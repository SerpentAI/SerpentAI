...
try:
    with open("config/config.private.yml", "r") as f:
        try:
            config_private = yaml.safe_load(f) or {}
        except yaml.scanner.ScannerError as e:
            raise Exception("Configuration file at 'config/config.private.yml' contains invalid YAML...")
        except Exception as e:
            print(type(e))
except FileNotFoundError as e:
    raise Exception("Configuration file not found at: 'config/config.private.yml'...")
...
config = {**plugin_config, **config_private, **config}
